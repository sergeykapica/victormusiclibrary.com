...
<div class="shop-sections-wrapper">
    <?if(isset($arResult['SECTIONS']) && !empty($arResult['SECTIONS'])):?>
        <ul class="shop-sections">
            <?foreach($arResult['SECTIONS'] as $sectionKey => $sectionValue):?>
                <li class="shop-section-item <?=( ( ( $sectionKey + 1 ) % 3 ) !== 0 ? 'section-item-indentation' : '' );?>">
                    <a href="/shop/<?=$sectionValue['CODE'];?>/" class="section-item-link">
                        <div style="background-image: url(<?=$sectionValue['DETAIL_PICTURE'];?>)" class="section-item-icon"></div>
                        <div class="section-item-content"><?=$sectionValue['NAME'];?></div>
                    </a>
                </li>
            <?endforeach;?>
        </ul>
    <?else:?>
        <p class="result-empty"><?=GetMessage('SHOP_SECTION_EMPTY');?></p>
    <?endif;?>
</div>
